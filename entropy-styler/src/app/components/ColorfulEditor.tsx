...
// Store word probabilities to maintain consistency
const wordProbabilities = new Map<string, number>();

// This will be replaced with actual LLM API call
const mockGetTokenProbability = async (word: string): Promise<number> => {
  // Check if we already have a probability for this word
  const existingProb = wordProbabilities.get(word.toLowerCase());
  if (existingProb !== undefined) {
    return existingProb;
  }

  // Generate and store new probability
  const newProb = Math.random();
  wordProbabilities.set(word.toLowerCase(), newProb);
